...
- **Type coercion**: Socrata returns every numeric field as a _string_. A test must prove the code casts explicitly and never relies on JS coercion.
...
5. **Honesty of presentation (NFR-5):** the reporting-affected collision series must carry both a dashed stroke **and** an explicit inline label — never color alone. Page copy must use correlation language; any wording asserting that enforcement _caused_ a change in deaths is a critical violation.

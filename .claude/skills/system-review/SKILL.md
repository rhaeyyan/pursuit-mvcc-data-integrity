...
- **Credentials:** confirm expected env keys exist by _name only_ (e.g. `grep -oE '^[A-Z_]+' .env` — never echo values into the report) and flag obvious placeholders (docs-example URLs, malformed keys) as P1 issues before they burn a session.

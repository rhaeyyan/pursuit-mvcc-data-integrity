...
## Archived 2026-08-08 — FR-6/FR-7 Phase 1 of 6: borough vocabulary + transport (COMPLETE)

**Outcome:** delivered 2 of 2 budgeted files (`src/lib/boroughs.ts` new, `src/lib/socrata.ts`
edited +23/−9). Cypress audit **PASS**, no critical violations. 478/478 tests, `tsc --noEmit`
clean, eslint 0 errors, `guard-data-integrity.sh` exit 0, `npm audit` 0 vulnerabilities — all on
Node 22.23.2. No test file modified; no dependency added; no third file touched.

**Byte-identity was computed, not asserted.** Cypress extracted HEAD via `git archive` and the
working tree via `rsync` into two scratch trees, dumped all four frozen FR-8 contracts plus the
2- and 3-arg builder outputs from each, and diffed the JSON: identical. It also confirmed the
test pins are full-string `.toBe()` comparisons against literals declared in the test file, not
`.toContain()` prefixes read back from the module under test.

**Trap 2 re-verified live rather than from recollection:** the five `h9gi-nx95` spellings match
`crashesValue` byte for byte, and an independent `arrest_boro` enumeration over Q1 2019 returned
`{B, K, M, Q, S}` with Brooklyn (`K`, 15,809) above the Bronx (`B`, 13,410) — corroborating
`B`=Bronx by row count.

**HITL decision 4 (dispatch approval) is spent and archived with this phase.** Decisions 1–3
remain binding on unbuilt phases and stay in `SPEC.md`.

Cedar's original Phase 1 contract, verbatim:

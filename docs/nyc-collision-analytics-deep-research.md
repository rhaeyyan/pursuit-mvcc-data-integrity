...
4. **A citation overclaimed a root cause.** The "Aggregate Nullification Anomaly" row (the
   ...
   _cause_ is unconfirmed speculation dressed up as a citation. Separately, the report's own
   _proposed remedy_ for this row — summing the subgroup fields as a fallback total — has since
   been independently falsified; see [ADR 0002](adr/0002-no-synthetic-subtotal-fallback.md).

**What was _not_ independently re-verified**, and should be treated as unverified rather than
...
pairs. That's _why_ it can process massive point sets in a single pass where K-Means needs the
...
— i.e. the collision dataset alone is _not_ sufficient for equity-aware policy analysis, an
...
- **Policy makers:** modernize MV-104AN's _digital_ entry interface with standardized
...
- **Rule 5 (Deterministic Rehearsal)** — the fact-check that produced this note's corrections
  _was_ a deterministic rehearsal: live API/metadata queries run before trusting the report's
  claims, not after.
...
- **Willow (Tutor Assistant)** — the "verify before distilling" pattern here is also directly
  ...
  and cites sources is not the same as a _verified_ answer.

...
// src/app/api/collisions/route.ts, per Rule 4: this tests the Route
// Handler's JSON response shape given a stubbed fetchCollisionsPerYear()
// result — not its fetch plumbing, which src/lib/collisions.test.ts already
// covers. Mirrors src/app/api/injuries/route.test.ts's structure exactly
// (the most recent generation of this pattern), per the dispatch
// instructions, substituting the collisions module/alias.
//
...
//
// fetchCollisionsPerYear() is mocked here on purpose — its own contract (ok /
// empty / error-upstream / error-contract) is already exercised in
// src/lib/collisions.test.ts. Re-testing it here would duplicate that
// coverage and couple this file to collisions.ts's internals rather than to
// route.ts's public HTTP contract.

import { afterEach, describe, expect, it, vi } from "vitest";

const { fetchCollisionsPerYear } = vi.hoisted(() => ({
  fetchCollisionsPerYear: vi.fn(),
}));

vi.mock("../../../lib/collisions", () => ({ fetchCollisionsPerYear }));
...
describe("GET /api/collisions — the union-to-HTTP mapping", () => {
  it('maps status: "ok" to HTTP 200 with { status, soql, rows }', async () => {
    const rows = [{ year: 2018, collisions: 1000 }];

...
// src/app/api/repaired-collisions/route.ts, per Rule 4: this tests the Route
// Handler's JSON response shape given a stubbed
// fetchRepairedCollisionsPerYear() result — not its fetch plumbing, which
// src/lib/repairedCollisions.test.ts already covers. Mirrors
// src/app/api/collisions/route.test.ts's structure exactly (the closest
// existing example — SPEC.md's dispatch instructions), substituting the
// repaired-collisions module/alias.
//
// `// @vitest-environment node` is the same pre-authorized fix the three
// existing route tests use: constructing a Web Response under the default
...
//
// fetchRepairedCollisionsPerYear() is mocked here on purpose — its own
// contract (ok / empty / error-upstream / error-contract) is already
// exercised in src/lib/repairedCollisions.test.ts. Re-testing it here would
// duplicate that coverage and couple this file to repairedCollisions.ts's
// internals rather than to route.ts's public HTTP contract.

import { afterEach, describe, expect, it, vi } from "vitest";

const { fetchRepairedCollisionsPerYear } = vi.hoisted(() => ({
...
describe("GET /api/repaired-collisions — the union-to-HTTP mapping", () => {
  it('maps status: "ok" to HTTP 200 with { status, soql, rows }', async () => {
    const rows = [{ year: 2018, repaired: 150 }];
